...
import './vcf-enhanced-select-overlay.js';
import './vcf-enhanced-select-text-field.js';
import '../theme/lumo/vcf-enhanced-select.js';
...
$_documentContainer.innerHTML = `<custom-style>
  <style>
    @font-face {
      font-family: "vcf-enhanced-select-icons";
...
/**
 *
 * `<vcf-enhanced-select>` is a Web Component for selecting values from a list of items. The content of the
 ...
 *
 * ```html
 * <vcf-enhanced-select id="select"></vcf-enhanced-select>
 * ```
 * ```js
 ...
 *
 * `<vcf-enhanced-select>` element sets these custom CSS properties:
 *
 * Property name | Description | Theme for element
 * --- | --- | ---
 * `--vcf-enhanced-select-text-field-width` | Width of the select text field | `vcf-enhanced-select-overlay`
 *
 * See [ThemableMixin – how to apply styles for shadow parts](https://github.com/vaadin/vaadin-themable-mixin/wiki)
 *
 * In addition to `<vcf-enhanced-select>` itself, the following internal
 * components are themable:
 *
 * - `<vcf-enhanced-select-text-field>`
 * - `<vcf-enhanced-select-overlay>`
 *
 * Note: the `theme` attribute value set on `<vcf-enhanced-select>` is
 ...
 */
class VcfEnhancedSelectElement extends
  ElementMixin(
    ControlStateMixin(
      ThemableMixin(
  ...
  static get template() {
    ...
    <vcf-enhanced-select-text-field placeholder="[[placeholder]]" label="[[label]]" required="[[required]]" invalid="[[invalid]]" error-message="[[errorMessage]]" readonly\$="[[readonly]]" helper-text="[[helperText]]" theme\$="[[theme]]">
      <slot name="prefix" slot="prefix"></slot>
      <slot name="helper" slot="helper">[[helperText]]</slot>
      <div part="value"></div>
      <div part="toggle-button" slot="suffix" role="button" aria-haspopup="listbox" aria-label="Toggle"></div>
    </vcf-enhanced-select-text-field>
    <vcf-enhanced-select-overlay opened="{{opened}}" with-backdrop="[[_phone]]" phone\$="[[_phone]]" theme\$="[[theme]]"></vcf-enhanced-select-overlay>
...
`;
  ...
  static get properties() {
    return {
      ...
      /**
       * Custom function for rendering the content of the `<vcf-enhanced-select>`.
       * Receives two arguments:
       *
       * - `root` The `<vcf-enhanced-select-overlay>` internal container
       *   DOM element. Append your content to it.
       * - `select` The reference to the `<vcf-enhanced-select>` element.
       ...
       * the values of inner vaadin-items, or set the vcf-enhanced-select value to
  ...
  ready() {
    ...
    this._overlayElement = this.shadowRoot.querySelector('vcf-enhanced-select-overlay');
  ...
  get focusElement() {
    return this._inputElement ||
      (this._inputElement = this.shadowRoot.querySelector('vcf-enhanced-select-text-field'));
  }
  ...
  _setPosition() {
    ...
    this._overlayElement.updateStyles({'--vcf-enhanced-select-text-field-width': inputRect.width + 'px'});
  }
   ...
   */
    ...
    static _finalizeClass() {
        ...
        if (typeof licenseChecker === 'function') {
            licenseChecker(VcfEnhancedSelectElement);
        }
    }
}

customElements.define(VcfEnhancedSelectElement.is, VcfEnhancedSelectElement);

export { VcfEnhancedSelectElement };
